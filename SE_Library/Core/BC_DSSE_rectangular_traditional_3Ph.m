function[Vr, Vx, Ir, Ix, num_iter] = BC_DSSE_rectangular_traditional_3Ph(num_nodes,nominal_linedata, ...
...
while (epsilonV > 10^-7 && num_iter <= 100)
    ...
    I_complex = complex(IrA(briampfA),IxA(briampfA));
    phi_Imag_br = angle(I_complex);
    ...
    I_complex = complex(IrB(briampfB),IxB(briampfB));
    phi_Imag_br = angle(I_complex);
    ...
    I_complex = complex(IrC(briampfC),IxC(briampfC));
    phi_Imag_br = angle(I_complex);
    ...
    Icomplex = [IcomplexA; IcomplexB; IcomplexC];
    
    VA(2:end) = VA(1) - Znod_state(1:num_A_nodes-1,:)*Icomplex;
    VB(2:end) = VB(1) - Znod_state(num_A_nodes:num_A_nodes+num_B_nodes-2,:)*Icomplex;
    VC(2:end) = VC(1) - Znod_state(num_A_nodes+num_B_nodes-1:end,:)*Icomplex;
